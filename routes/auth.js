...
router.get('/discord/callback', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('Rôles Discord non récupérés - rôle par défaut: membre');
    }
    ...
    // Stocker dans un cookie signé (pas de session serveur)
    ...
    console.log(`✓ Connexion réussie: ${discordUser.username} (${siteRole})`);
...
router.post('/logout', (req, res) => {
  res.clearCookie('gs_user');
  res.json({ success: true });
});

router.get('/sync-discord', (req, res) => {
  res.json({ success: true });
});
